...
vector<Tokens> Analyzer::getParsableTokens(string file_name) {
  ...
  while (file_string.size() > 0) {
    ...
    if (current_token != WHITESPACE && current_token != COMMENT) {
      Tokens new_token;
      new_token.type = current_token;
      new_token.contents = file_string.substr(0, character_count);
  ...
  eof_token.line_number = line_number;
...
string Analyzer::enumToString(token token_type) {
  switch(token_type) {
    case COMMA:
      return "COMMA";
    case PERIOD:
